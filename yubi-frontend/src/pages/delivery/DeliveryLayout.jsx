...
export default function DeliveryLayout() {
  ...
  return <div style={{ color: "#1A1A1A" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {isMobile && (
          <button
            ...
            onClick={toggle}
            disabled={busy}
            style={{ ...headerSwitch, background: online ? "#4CAF50" : "#9CA3AF", opacity: busy ? 0.7 : 1 }}
            aria-label="Toggle online for admin"
          >
            <span style={{ ...switchThumb, transform: online ? "translateX(20px)" : "translateX(0)" }} />
          </button>
        )}
    ...
    {!isMobile && <aside style={{ ...sidebar, width: sideWidth }}>
      ...
      {isOpen && <StatusPanel partner={partner} online={online} onToggle={toggle} busy={busy} />}
    </aside>}
...
function StatusPanel({ partner, online, onToggle, busy }) {
  return <div style={statusPanel}>
    ...
    <button type="button" onClick={onToggle} disabled={busy} style={{ ...switchTrack, background: online ? "#4CAF50" : "#9CA3AF", opacity: busy ? 0.75 : 1 }} aria-label="Toggle online status">
...
const headerSwitch = { border: "none", borderRadius: 999, width: 48, height: 28, padding: 3, cursor: "pointer", display: "flex", alignItems: "center", transition: "background .2s ease", flexShrink: 0 };
